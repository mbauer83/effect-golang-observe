...
func TestRunningFibersAreNestedAsTheyWereForked(t *testing.T) {
	...
	running := fibers.Running()
	if len(running) != 1 || running[0].ID != 1 {
		t.Fatalf("expected the forker as the one root, got %v", running)
	}
	if len(running[0].Children) != 2 {
		t.Fatalf("expected both forked fibers under it, got %v", running[0].Children)
	}
	// Oldest first: the one that has been running longest is the one to look
	// at.
	if running[0].Children[0].ID != 2 || running[0].Children[1].ID != 3 {
		t.Fatalf("expected them in the order they were forked, got %v", running[0].Children)
	}
	if fibers.Count() != 3 {
...
func TestAFiberIsForgottenWhenItCompletes(t *testing.T) {
	...
	if started, ended := fibers.Started(), fibers.Ended(); started != 2 || ended != 1 {
...
func TestAFiberOutlivingItsForkerIsARootRatherThanAHoleInTheTree(t *testing.T) {
	...
	if len(running) != 1 || running[0].ID != 2 {
		t.Fatalf("expected the outliving fiber as a root, got %v", running)
	}
	// The parent it named is kept, because it is true and says where the work
	// came from.
	if running[0].ParentID != 1 {
		t.Fatalf("expected the forker it named kept, got %d", running[0].ParentID)
	}
}
...
func TestAFibersAgeIsHowLongItHasBeenRunning(t *testing.T) {
	...
	running := fibers.Running()
	if age := running[0].Age(now); age != 4*time.Minute {
...
func TestAnOpenSpansAgeIsCountedFromNowAndAFinishedOnesIsWhatItTook(t *testing.T) {
	running := trace.Watch()
	running.Observe(context.Background(), spanStarted(1, 0, "waiting", 0))
	...
	if age := assembled.Roots[0].Age(at(time.Hour)); age != time.Millisecond {
